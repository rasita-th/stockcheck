...
def build_payload(
    ...
    **kwargs: Any,
    ...
    payload.setdefault("policy", {})["source_policy_phase"] = (
        "P5 requires canonical provenance and domain policy; legacy fallback is retired"
    )
    return payload
...
def main() -> None:
    ...
    print(
        "Generated P5 earnings radar: "
        f"{payload['coverage']['published_rows']} rows / "
        f"{metrics['canonical_rows']} canonical / 0 legacy fallback"
    )
