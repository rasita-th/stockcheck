(() => {
  ...
  function makeAction(id, label, icon, href) {
    ...
    if ((id === 'market' && isMarketPage()) || (id === 'scanner' && !isMarketPage())) element.setAttribute('aria-current', 'page');
    return element;
  }
  ...
  function updateBadge() {
    const today = $('.sr945-primary-nav [data-sr945-action="today"]');
    if (!today) return;
    const count = readAlertCount();
    let badge = $('.sr945-nav-badge', today);
    if (!count) { badge?.remove(); return; }
    if (!badge) { badge = document.createElement('span'); badge.className = 'sr945-nav-badge'; today.appendChild(badge); }
  ...
  function openScanner() {
    if (isMarketPage()) return location.assign('index.html#scanner');
    if (!clickNativeView('scanner')) ($('.scanner-panel') || $('#technicalScanner') || $('main'))?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }
  function openToday() {
    if (isMarketPage()) return location.assign('index.html#today');
    if (!clickNativeView('attention')) { $('[data-alert-filter="all"]')?.click(); $('#alertCenter')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }
  }
  function openMemo() {
    if (isMarketPage()) return location.assign('index.html#memo');
    if (!clickNativeView('memo')) { $('[data-alert-filter="memo"]')?.click(); $('#alertCenter')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }
  }
  function applyHash() {
  ...
  function mount() { buildNavigation(); applyHash(); setTimeout(updateBadge, 600); setTimeout(updateBadge, 1800); }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', mount, { once: true }); else mount();
  addEventListener('load', mount, { once: true });
  addEventListener('hashchange', applyHash);
  new MutationObserver(updateBadge).observe(document.documentElement, { childList: true, subtree: true, characterData: true });
