(() => {
  ...
  function openScanner() {
    if (isMarketPage()) return location.assign('index.html');
    ($('.scanner-panel') || $('#technicalScanner') || $('main'))?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  function openAlerts(filter) {
    if (isMarketPage()) return location.assign(`index.html#${filter}`);
  ...
  document.addEventListener('click', event => {
    ...
    handleAction(control.dataset.sr94Action);
  });

  function mount() { buildNavigation(); openHash(); }
  ...
  const observer = new MutationObserver(() => {
    ...
    } else if (badge && count) badge.textContent = count;
  });
