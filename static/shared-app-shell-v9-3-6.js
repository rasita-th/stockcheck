(() => {
  ...
  function ensureNativeNavigation() {
    ...
    if (!market) {
      ...
      market.innerHTML = '<span aria-hidden="true">🌍</span><span>Market Pulse</span>';
    }
  ...
  document.addEventListener('click', event => {
    const retry = event.target.closest?.('[data-sr946-retry]');
    if (retry) {
  ...
  window.StockRadarShellV946 = {
    version: VERSION,
    boot,
    ensureNativeNavigation,
    verifyDataAndRecover,
    activateView,
    setViewState,
  };
